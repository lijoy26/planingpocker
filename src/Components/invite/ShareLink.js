...
function ShareLink() {
  ...
  useEffect(() => {
    ...
    const kite = `${linkz}/poker?name=&room=${type}&cardVale=${cal}`;
    sk(kite);
  }, []);
    ...
    <div className="Share" onBlur={() => setCopied(false)}>
      ...
      >
        <div className="popc">
          <textarea
            className="pop"
            style={{ fontSize: '11px', fontWeight: 'bold', padding: '5px' }}
            value={k}
            readOnly
          ></textarea>
          <button
            aria-hidden="true"
            className="copyText"
            style={{ color: '#c10e21', fontWeight: 'bold', cursor: 'pointer', background: 'none', border: 'none'}}
            onClick={handleCopy}
          >
            <i className="fa fa-clipboard" id="tag"></i>
          </button>
        </div>
        {copied ? (
          <div
            className="copied"
            style={{ color: '#c10e21', fontWeight: 'bold', fontSize: '12px', paddingTop: '5px', paddingLeft: '2px' }}
          >
            Copied to Clipboard
          </div>
        ) : null}
